...
import { Workshop, Attendee, ProjectStatus, WorkshopView } from '../types';
...
const statusClassMap: Record<ProjectStatus, string> = {
  [ProjectStatus.SHIPPED]: 'bg-emerald-50 text-emerald-600',
  [ProjectStatus.IN_PROGRESS]: 'bg-amber-50 text-amber-600',
  [ProjectStatus.NOT_STARTED]: 'bg-slate-100 text-slate-600'
};
            ...
            <tbody className="divide-y divide-slate-100">
              {workshop.attendees.map((attendee) => (
                <tr key={attendee.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-5">
                    <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${statusClassMap[attendee.status]}`}>
                      {attendee.status}
                    </span>
                  </td>
                  <td className="px-6 py-5 text-center">
                    <div className={`text-sm font-bold ${attendee.engagementScore > 80 ? 'text-indigo-600' : 'text-slate-400'}`}>
              ...
              {workshop.attendees.length === 0 && (
                <tr>
                  <td className="px-8 py-6 text-sm text-slate-400" colSpan={5}>
                    No attendees yet for this workshop.
                  </td>
                </tr>
              )}
            </tbody>
